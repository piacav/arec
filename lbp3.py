...
def lbp3_fun(age_param, divisions, classifier):
    ...
    cascade = cv2.CascadeClassifier(cv2.data.haarcascades + classifier)
        ...
        for e in train_images:
            ...
            for i in range(divisions):
                ...
                for j in range(divisions):
                    ...
                    # Create histogram
                    (hist, _) = np.histogram(fragment.ravel(), bins=np.arange(0, no_points + 3),
                                             range=(0, no_points + 2))
                    if len(conchist) == 0:
                        conchist = hist
                    else:
    ...
    out = '\n' + line + output_tot + '\n' + line + output_age + '\n' + line + output_gen + '\n' + line
    # plt.show()
    remove('lbp_model_age.pkl')
    remove('lbp_model_gen.pkl')
    return (line, output_tot, output_age, output_gen)
